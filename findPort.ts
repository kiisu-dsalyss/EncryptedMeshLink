...
export async function findPort(): Promise<string> {
    console.log("🔍 Scanning for Meshtastic devices...");
    
    const ports = await SerialPort.list();
    
    if (ports.length === 0) {
        throw new Error("No serial ports found on this system.");
    }
    ...
    if (bestPort.score === 0) {
        throw new Error("No likely Meshtastic devices found. Please check your device connection.");
    }

    // If there's a clear winner (score > 50), use it
    if (bestPort.score >= 50) {
        console.log(`✅ Auto-selected: ${bestPort.port.path}`);
    ...
    console.log(`⚠️  No high-confidence match found. Using best guess: ${bestPort.port.path}`);
    console.log("   If this doesn't work, you may need to set MESHTASTIC_PORT manually in .env");
